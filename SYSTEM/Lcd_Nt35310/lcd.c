...
//size:字体大小 12/16/24
//mode:叠加方式(1)还是非叠加方式(0)
void LCD_cShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode,u16 color)
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_cShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p,u16 color)
{         
	u8 x0=x;
	width+=x;
	height+=y;
    ...
    {       
        if(x>=width){x=x0;y+=size;}
        if(y>=height)break;//退出
        LCD_cShowChar(x,y,*p,size,0,color);
        x+=size/2;
        p++;
    }  
}
