...
#include "delay.h"
#include "usart.h"
//字库区域占用的总扇区数大小(6个字库+unigbk表+字库信息=3238700字节,约占3193个W25QXX扇区)
...
//字库存放在磁盘中的路径
u8*const GBK64_PATH=(u8 *)"/FONT/GBK64.FON";		//GBK64的存放位置
u8*const GBK56_PATH=(u8 *)"/FONT/GBK56.FON";		//GBK56的存放位置
u8*const GBK48_PATH=(u8 *)"/FONT/GBK48.FON";		//GBK48的存放位置
u8*const GBK32_PATH=(u8 *)"/FONT/GBK32.FON";		//GBK32的存放位置
 	...
 	if(rval==0)	 
	{
		switch(fx)
		{
			...
			case 3:
				...
				break;
			case 4:
				ftinfo.f32addr=ftinfo.f24addr+ftinfo.gkb24size;	//GBK24之后，紧跟GBK32字库
				ftinfo.gkb32size=fftemp->fsize;					//GBK32字库大小
				flashaddr=ftinfo.f32addr;						//GBK32的起始地址
				break;
			case 5:
				ftinfo.f48addr=ftinfo.f32addr+ftinfo.gkb32size;	//GBK32之后，紧跟GBK48字库
				ftinfo.gkb48size=fftemp->fsize;					//GBK48字库大小
				flashaddr=ftinfo.f48addr;						//GBK48的起始地址
				break;
//			case 6:
//				ftinfo.f56addr=ftinfo.f48addr+ftinfo.gkb48size;	//GBK48之后，紧跟GBK56字库
//				ftinfo.gkb56size=fftemp->fsize;					//GBK56字库大小
//				flashaddr=ftinfo.f56addr;						//GBK56的起始地址
//				break;
//			case 8:
//				ftinfo.f64addr=ftinfo.f56addr+ftinfo.gkb56size;	//GBK56之后，紧跟GBK64字库
//				ftinfo.gkb64size=fftemp->fsize;					//GBK64字库大小
//				flashaddr=ftinfo.f64addr;						//GBK64的起始地址
//				break;
		} 
...
{	
	...
	strcat((char*)pname,(char*)GBK48_PATH); 
 	res=f_open(fftemp,(const TCHAR*)pname,FA_READ); 
 	if(res)rval|=1<<9;//打开文件失败 
//	strcpy((char*)pname,(char*)src);	//copy src内容到pname
//	strcat((char*)pname,(char*)GBK56_PATH); 
// 	res=f_open(fftemp,(const TCHAR*)pname,FA_READ); 
// 	if(res)rval|=1<<10;//打开文件失败 
//	strcpy((char*)pname,(char*)src);	//copy src内容到pname
//	strcat((char*)pname,(char*)GBK64_PATH); 
// 	res=f_open(fftemp,(const TCHAR*)pname,FA_READ); 
// 	if(res)rval|=1<<11;//打开文件失败
	free(fftemp);//释放内存
	if(rval==0)//字库文件都存在.
	{  
		...
		strcpy((char*)pname,(char*)src);				//copy src内容到pname
		strcat((char*)pname,(char*)GBK48_PATH); 
		res=updata_fontx(x+20*size/2,y,size,pname,5);	//更新GBK48.FON
		if(res){free(pname);return 6;}
//		LCD_ShowString(x,y,240,320,size,(u8 *)"Updating GBK56.BIN  ");
//		strcpy((char*)pname,(char*)src);				//copy src内容到pname
//		strcat((char*)pname,(char*)GBK56_PATH); 
//		res=updata_fontx(x+20*size/2,y,size,pname,6);	//更新GBK56.FON
//		if(res){free(pname);return 7;}
//		LCD_ShowString(x,y,240,320,size,(u8 *)"Updating GBK64.BIN  ");
//		strcpy((char*)pname,(char*)src);				//copy src内容到pname
//		strcat((char*)pname,(char*)GBK64_PATH); 
//		res=updata_fontx(x+20*size/2,y,size,pname,7);	//更新GBK64.FON
